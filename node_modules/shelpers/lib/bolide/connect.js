...
function DelayInputCommand() {
    ...
    p.loadConfig = function loadConfig(options) {
        ...
        settings.args = args;
        var delayedArgs =   sh.splitStrIntoArray(options.delayedArgs);
        ...
        cR.execute(settings)
        cR.terminal.stdin.setEncoding('utf8');

        //console.log( cR.terminal.stdin )
        var wait = 3000
        setTimeout( function ag(){
            sh.each(delayedArgs, function argX(i, arg){
                setTimeout(function callC(){
                    console.log(arg+'\n')
                    cR.terminal.stdin.write('dfg\n')
                  // console.log( cR.terminal.stdin )
                   // cR.terminal.stdin.end()
                    cR.write(arg)
                }, i*wait);
...
if (module.parent == null) {
    ...
    function sdf() {
        ...
        var options = {}
        options.cmd = 'sudo'
        options.args = 'openvpn, /etc/openvpn/US\ East.ovpn'
        options.cmd = '/etc/openvpn'
        options.cmd = 'openvpn'
        options.args = ['"/etc/openvpn/US\ East.ovpn"']
        options.args = ['/etc/openvpn/US\ East.ovpn']
        options.delayedArgs = "p8666068, snTFsTGh7K"
        //options.arg2 = 'fff'
