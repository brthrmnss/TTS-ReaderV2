...
function readFile(file, nullValue, binary ) {
    ...
    try {
        ...
        console.error('cannot find', __dirname , file, require('path').resolve(__dirname+'/'+file))
        throw e
    }
    return contents;
}
...
sh.fs.getFilePath = function getFileNameOnly(file) {
    var path = require("path");
    var fs = require("fs")
...
sh.fs.getFileNameOnly = function getFileNameOnly(file) {
    var path = require("path");
...
sh.fs.readJSONFile = function readJSONFile(file, defaultVal, ifSwallowErrors ) {
    ...
    if ( file == null ) { if ( ifSwallowErrors == true ) {
        return defaultVal;
    }};
    if ( sh.fileExists(file)==false) {
...
sh.fs.appendJSON = function (file, append) {
    //treat file path as json-based stream/log
    //get file create if not exists
    var json = sh.fs.readJSONFile(file, [], true)
    json.push(append)
    sh.writeJSONFile(file, json)
    //return json;
}
