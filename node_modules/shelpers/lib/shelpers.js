...
str.isBlank = function isBlank(str, msg) {
    if (str == '' || str == null) {
        if (msg) {
            sh.throw(msg)
        } //throw error
...
str.prepend = function prependToStr(str, prepend, ifNull) {
    if (str == null) {
        if (ifNull !== undefined) {
            return ifNull;
        }
...
str.template = function template(str, marker, replaceWith) {
    var result = str;
    result = result.replace(marker, replaceWith)
    return result;

}

str.isLowerCase = function isLowerCase(char) {
    if (char == null) {
        return null
    }
    if (char == "\n" && char.toLowerCase == null) {
        return false
        debugger
    }
    if (sh.isString(char) && char.toLowerCase == null) {
        return false;
        console.error('no lower case', char, char.toString())
        debugger
    }
...
str.isUpperCase = function isUpperCase(char) {
    if (char == null) {
        return null
    }
...
str.isFirstLetterLowercase = function isFirstLetterLowercase(txt) {
...
str.isCapitalized = str.isFirstLetterCapitalized = str.isFirstLetterUppercase = function isFirstLetterUppercase(txt) {
    if (txt == null) {
        return null
    }
    var char = txt.slice(0, 1)
...
str.words = str.splitStringIntoWords = function word(strInput) {
    var words = strInput.split(/(\s+)/);
    var words2 = []
    $.each(words, function removeEmptyStrings(k,word) {
        if ( word.trim() == '' ) {
            return;
        }
        words2.push(word)
...
sh.changeDir = sh.chdir = sh.cwd = sh.changeWorkingDirectory = function changeWorkingDiriector(toDir) {
    if (toDir)
        process.chdir(toDir);
    return process.cwd()
}
...
sh.isAnyInAny = function isAnyInAny_Str(query, arr, caseSensitive) {
    var found = null
...
sh.isAnyInAny2 = function isAnyInAny(qArray, mArray, caseSensitive) {
    var matches = [];
    var found = null
    sh.each(qArray, function testIfQueryInStr(k, query) {
        query = query.trim()
        if (caseSensitive === true) {
        } else {
            query = query.toLowerCase();
        }
        sh.each(mArray, function testIfQueryInStr(k, mItem) {
            mItem = mItem.trim()
            if (caseSensitive === true) {
            } else {
                mItem = mItem.toLowerCase();
            }
            if (mItem == '')
                return;
            if (mItem == query) {
...
function replaceForwardBackslash(str) {
    var replaced = str.replace(/\//gi, "\\");
    return replaced
}
...
function DictArray() {
    ...
    p.add = function addItemToDictionary(key, val) {
        ...
        if ( self.settings.allowDupes != true ) {
    ...
    p.getKeys = function getKeys() {
        var keys = sh.args(arguments)
        var outputList = [];
        sh.each(self.data.dict, function addKeyToOutputX(dictKey, v) {
            if (keys.includes(dictKey)) {
                outputList = outputList.concat(v)
                console.log(sh.t, 'add', dictKey, v.length)
            }
    ...
    p.getKeysNamesOpposite = function getKeysNamesOpposite() {
        var keySkip = sh.args(arguments)
        var keyList = [];
        sh.each(self.data.dict, function addKeyToOutputX(dictKey, v) {
            if (keySkip.includes(dictKey)) {
                return;
            }
            if (keyList.includes(dictKey) == false) {
                keyList.push(dictKey);
            }
    ...
    p.getInverse = function getInverseAnyItemsNotInArray(arr) {
        var dict = {}
        var flatList = self.flatten()
        /*sh.each(flatList, function addtoDict(k,v) {
         //v.indexY = k;
         dict[k] = v;
         })

         sh.each(arr, function removeExisting(k,v){
         delete dict[v]
         })

         var outputList = []
         sh.each(dict, function addToDict(k,v){
         outputList.push(v)
         })*/
    ...
    p.peak = function onPeak(k) {
        //var self = h.data.dictFlagged;
        var arr =
            self.data.dict[k]
        if (arr && arr.length > 0) {
            console.log(arr[0])
    ...
    p.tagItems = function tagItemsByGrouping() {
        sh.each(self.data.dict, function addKeyToOutputX(dictKey, arrItems) {
            sh.each(arrItems, function tagEachItem(k, item) {
                item.tag = dictKey;
            })
        });
    }

    p.flatten = function flattenDict() {
        var outputList = []
        var outputList2 = self.data.dict;
        sh.each(outputList2, function removeField(arrKey, arrItems) {
            sh.each(arrItems, function removeField(k, item) {
    ...
    p.addSet = function addSet(list, prop, only1PerKey) {
        sh.each(list, function addEactItem(arrKey, item) {
            var key = item[prop]
            if (only1PerKey) {
    ...
    p.sum = function sum(propName) {
        ...
        sh.each(outputList2, function removeField(arrKey, arrItems) {
            sh.each(arrItems, function removeField(k, item) {
                outputList.push(item)
                var val = item[propName]; //.totalSize;
                if (sh.isNumber(val) && isNaN(val) == false) {
                    totalSum += val;
                }
            })
        })
    ...
    p.sum2 = function sum2AndRoundOff(asdf, postAmble) {
        postAmble = sh.dv(postAmble, '')
        return sh.toNum(self.sum(asdf)) + ' ' + postAmble
    }


    self.getFirst = function getFirst(key) {
        var arr = self.data.dict[key]
        var first = null;
        if (arr) {
            first = arr[0]
    ...
    self.includes = function includes(key) {
        return self.data.dict[key] != null
    }

    self.addIfNotPresent = function addIfNotPresent(key, item, fxNotPresent, fxIfPresent) {
        var hasKey = self.includes(key)

        if (hasKey) {
            sh.cid(fxIfPresent, item)
        } else {
            self.add(key, item)
            sh.cid(fxNotPresent, item)
        }
        //return hasKey
    }

    self.if = function ifHasKeyDoFx(key, fx) {
        var hasKey = self.includes(key)
        if (hasKey) {
            fx(self.data.dict[key], key)
        }
        return
    }
    ...
    self.print = function print(key, val) {
        var i = 0;
        sh.each(self.data.dict, function print(k, v) {
            i++
            console.log(i, k)
        })
    }
    self.printDupes = function printDupes(fxExtra) {
        var i = 0;
        sh.each(self.data.dict, function print(k, v) {
            if (v.length <= 1) {
                return;
            ...
            i++
            var val = sh.cid(fxExtra, v)
            console.log(i + '.', k, v.length, val)
        })
    }

    self.filter = function filter(fxExtra) {
        var i = 0;
        var dictFiltered = {}
        sh.each(self.data.dict, function print(k, v) {
            if (v.length <= 1) {
        ...
        self.data.dict = dictFiltered;
        return dictFiltered;
    }

    self.each = function each(fx) {
        var i = 0;
        sh.each(self.data.dict, function callFx(k, v) {
            if (sh.isFunction(v)) {
                return;
            }
    ...
    self.each2 = function iterateInnerItems(fx) {
        var outputList2 = self.data.dict;
        var i = 0;
        sh.each(outputList2, function removeField(arrKey, arrItems) {
            sh.each(arrItems, function removeField(k, item) {
...
sh.toNull = function inputToNull(item, prop, prop2) {
    var output = 'null'
    if (item) {
        output = item;
        if (prop) {
...
sh.newDict = function createNewDict() {
    var n = new DictArray()
    return n;
}

sh.dict = sh.newDict;
sh.dict.prop = sh.dict.addItems = function onK(items, prop) {
    var dict = sh.newDict()
    dict.addSet(items, prop)
    return dict;
}
sh.dict.getFirst  = function getFirst(asdf) {
    var firstItem = null;
    sh.each(asdf, function ok(k,v) {
        firstItem = v;
        return false;
    })
    return firstItem
...
sh.displayInput = function displayInput() {
...
sh.require = function requireRelativeToRoot(file, justPath) {
    //console.error(file, __dirname)
...
sh.requirePath = function requirePath(file) {
    return sh.require(file, true);
}


sh.requirePathSH = function requirePath(file, justPath) {
    justPath = sh.dv(justPath, true);
    var file = sh.fs.join(__dirname, file);
    if (justPath) {
        return file;
    }
...
sh.require2 = function getClassMatchingName(dir, file) {
    //console.log('__dirname', __dirname)
    //console.log('__filename', __filename, process.argv[1])
    if (file == undefined && dir) {
        file = dir;
        dir = null;
        //
        if (file.includes('/') == false) {
            dir = sh.fs.path(process.argv[1])
        }
    }
    if (false == file.endsWith('.js')) {
        file += '.js'
    }
    if (dir) {
        var file = sh.fs.join(dir, file)
    } else {
        file = sh.fs.join(file)
    }
    /*if  ( justPath ) {
     return  file
     }*/
...
sh.require3 = function require3_GuessFromName(name) {
    //console.log(__dirname, module.parent, require.main)
    console.log(require.main.filename)
    var path = sh.fs.path(require.main.filename)
    console.log(path)
    var file = name;
    if (file.endsWith('.js') == false) {
        file += '.js'
    }
    var dir = sh.fs.join(path, name);
    console.log('path', dir)
    var clazz = require(dir)
    var y = clazz[name]
    return y;
...
sh.str.firstChar = function firstChar(str) {
    return str.slice(0, 1);
}

sh.str.lastChar = function lastChar(str) {
    return str.slice(-1)[0];
}

sh.str.firstNum = function firstNum7(str) {
    var chars = [];

    var noneString = false;

    sh.each(str, function eachChar(k, char) {
        if (sh.isNumber(char)) {
            if (noneString == true) {
                chars = null;
                console.log('ok', str, char)
...
log.iteration = function showLogOnIteration(iterationINdex, length, dividAt, addLog) {
    dividAt = sh.dv(dividAt, 100)
    if (length && length.length) {
        length = length.length;
    }
    if (iterationINdex % dividAt == 0) {
        console.log('at', iterationINdex,
            sh.percent(iterationINdex / length), addLog)
    }
}
...
log.enable = function enable() {
    ...
    console.log = log.fxOld
    log.fxOld = null;
    log.isDisabled = false;
}
log.disableCount = 0;
log.disable = function disable() {
    ...
    log.fxOld = console.log;
    console.log = function logx() {

    }
}

log.stack = sh.errors.showStack


function defineLogError() {

    log.error = {};
    log.error.enable = function enable() {
        if (log.fxErrorOld == null) {
            return;
        }
        console.error = log.fxErrorOld
        log.fxErrorOld = null;
        log.isErrorDisabled = false;
    }
    log.disableErrorCount = 0;
    log.error.disable = function disableError() {
        log.isErrorDisabled = true;
        log.disableErrorCount++;
        if (log.fxErrorOld) {
            return;
        }
        log.fxErrorOld = console.error;
        console.error = function logx() {
...
log.table = function logTable(input, display) {
    var columnify = require('columnify')
    var tOutput = columnify(input)
    if (display != false) {
        console.log(tOutput)
    }
    return tOutput;
...
log.fixX
log.file = function file(k, msg) {

    /*
     console.error('try hit file', new Error().stack)
     console.error('at step4_moveOutputFileToProject (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:324:31)')
     console.error('at step4_moveOutputFileToProject (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:324:31)')
     console.error('at step4_moveOutputFileToProject (g:/Dropbox/projects/crypto/ritv/imdb_movie_scraper/wrappers/imdb_app_v3_wrapper.js:324:31)')
     console.error('(g:/Dropbox/projects/crypto/ritv/imdb_movie_scraper/wrappers/imdb_app_v3_wrapper.js:324:31)')
     console.error('at step4_moveOutputFileToProject (g:/Dropbox/projects/crypto/ritv/imdb_movie_scraper/wrappers/imdb_app_v3_wrapper.js)')
     console.log('at step4_moveOutputFileToProject (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:324:31)')
     console.log('at dddd (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:324:31)')
     console.log('at (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:324:31)')
     console.log('at (g:\\Dropbox\\projects\\crypto\\ritv\\imdb_movie_scraper\\wrappers\\imdb_app_v3_wrapper.js:1:1)')

     */
    var path = sh.paren(k + ':1:1')
    var pre = 'at  ';
    console.error(pre, path)
    if (msg) console.error(sh.t, msg)
}
sh.fs.log = log.file

log.stopErrors =
    log.redirectErrors = function redirectErrors(fxDone, skipDebugger) {
        if (log.data.oldErrorFx) {
            return;
        }
        log.data.oldErrorFx = console.error;
        console.error = function onError() {
            var args = sh.args(arguments)
            if (fxDone) {
                fxDone.apply(console, args)
            }
            if (skipDebugger != true) {
                debugger; //so debugger can hit this
            }
        }
    }
log.resumeErrors = function resumeErrors() {
    console.error = log.data.oldErrorFx;
    log.data.oldErrorFx = null;
}

log.lines = log.newline = log.addNewline = log.n = function logCreateNewlines(count, showPosition) {
    count = sh.dv(count, 5)
    sh.each.times(count, function addNewLine(k, v) {
        console.log(sh.n)
    })
}

log.skipLines = function skipLines() {
    log.n(5)
}
...
sh.reload = {};
sh.reload.reloadFile = function reloadFile(___filename) {
    //  console.log('ok', ___filename, process.argv)
    if (process.argv[2] != 'selfFileMode') {
//sh.runWhenFileChanged(__dirname, 'AutoItServer.js')
        var fW = sh.runWhenFileChanged(___filename, 'selfFileMode*^!')
        //tell it to use this mode 
        //fW.settings.debugChanges = true
        fW.settings.countChanges = true
    }
}
sh.runWhenFileChanged = function runWhenFileChanged() { //reloader
    exports.reloadAny = true
    var fileX = '';
    var args = sh.args(arguments);
    var selfFileMode = false;
    if (args.length == 2 && args[1] == 'selfFileMode*^!') {
        console.log('self file mode, ensure check for looping')
        selfFileMode = true;
        args.pop()
    }
    if (args.length == 1) {
        fileX = args[0];
        var findRunnerStr = '_Runner';
        if (fileX.endsWith(findRunnerStr + '.js')) {
            fileX = fileX.replace(findRunnerStr, '')
        }
        findRunnerStr = findRunnerStr.toLowerCase();//
        if (fileX.endsWith(findRunnerStr + '.js')) {
            fileX = fileX.replace(findRunnerStr, '')
        }
    } else {
...
//sh.x()
    ...
    if (selfFileMode) {
        config.runParams = ['selfFileMode']
    }

    config.file = fileX;

    config.delayStart = true;
    f.init(config)
...
sh.errors.showStack = function showStack() {
    var err = new Error();
...
sh.later = function later(fx) {
    if (sh.isFunction(fx) == false) {
        sh.throw('need fx ' + fx)
    }
    setTimeout(function calLater() {
        fx()
    }, 1)
...
//simplifies invokation
    ...
    sh.fs.getFilesInDirectory3 = function getFilesInDirectory3(cfg) {

        var fs = require("fs")
        var filesAll = fs.readdirSync(cfg.dir);
        var filesWithDir = [];
        var files = []
        sh.each(filesAll, function goThroughFiles(k,file) {
           // var file = filesAll[i]
            var name = cfg.dir + '/' + file;
            try {
                //if (!filesAll.hasOwnProperty(i)) continue;
                ...
                console.error('no with', name)
                console.error('no with', e)
            }
        })

        if (cfg.filter != null) {
            files = files.filter(cfg.filter);
        }

        if (cfg.filterIncludes != null) {
            var matches = []
            sh.each(files, function processEach(i, fileName) {
                if (sh.includes(fileName, cfg.filterIncludes)) {
                    matches.push(fileName);
                }
            })
            files = matches;
        ...
        if (cfg.sortByTime) {
            var y = files.sort(function (a, b) {
                return fs.statSync(cfg.dir + '/'+a).mtime.getTime() -
                    fs.statSync(cfg.dir +'/'+ b).mtime.getTime();
            });
        }


        var sep = '/';
        if (sh.endsWith(cfg.dir, '/')) {
            sep = ''
        }
        if (cfg.addPath || cfg.appendDirOnFileNames == true) {
            files = files.map(function (fileName) {
                return cfg.dir + sep + fileName
            });
    ...
    sh.fs.readJSONFile = function readJSONFile(file, defaultVal, ifSwallowErrors) {
        ...
        try {
            ...
            console.error('parsing error with loading json', 'readJSONFile', file, e);
            sh.showLine('here is error')
            if (ifSwallowErrors != true) {
                throw e
            }
...
//Write temp file
    ...
    sh.fs.goUpDir = function goUpDir(pathStr, count) {
        count = sh.dv(count, 1)
        /*if ( count == null ) {
         count = 1
         }*/
        //  var pathStr = '';
        if (sh.isNumber(pathStr)) {
            count = pathStr;
            pathStr = '';
        }
        if (pathStr.endsWith('/') == false) {
            pathStr += '/'
        }
    ...
    sh.slash = sh.fs.slash = replaceBackslash;
    sh.fslash = sh.fs.fslash = replaceForwardBackslash;
    sh.fs.clean = stripSpecialChars;

    sh.fs.clean2 = function stripSpecialChars2(file) {
...
function defineFS() {
    ...
    sh.fs.parseBytesOutput = function parseSize_HumanReadable_CmdLineOutput_To_Gigabytes(cmdInput) {
        ...
        if (sh.endsWith(cmdInput, 'K')) {
            ...
            cmdOutput = parseFloat(cmdOutput);
            cmdOutput = cmdOutput / (1000 * 1000);
        }
    ...
    sh.fs.getSizeOfDir3 = function getSizeOfDir3(dirToCheck, fxDone) {
        var child_process = require('child_process');
        var cmd = "du -hsb " + sh.qq(dirToCheck);
        if (sh.isWin()) {
            cmd = 'dir /s /a ' + sh.qq(dirToCheck)
        }
        if (sh.fs.exists(dirToCheck) == false) {
            console.log('dir not exist')
            return -1;
        }
        var output3 = child_process.execSync(cmd)
        var output = output3.toString();

        if (sh.isWin()) {
            var TFL = 'Total Files Listed:';
            if (output.includes(TFL) == false) {
                console.error('bad cmd or bad size, output is wrong on', dirToCheck)
                return -1
            }
            var fileSizeOnly = sh.getContentAfter(output, TFL)
            var fileSizeOnly2 = sh.getContentAfter(fileSizeOnly, 'File(s)')
            fileSizeOnly2 = fileSizeOnly2.trim();
            var fileSizeOnly3 = sh.getContentBefore(fileSizeOnly2, 'bytes')
            fileSizeOnly3 = fileSizeOnly3.trim();
            fileSizeOnly3 = sh.replace(fileSizeOnly3, ',', '')
            output = parseInt(fileSizeOnly3)

        } else {
            var fileSizeOnly = sh.getContentBefore(output, '\t')
            output = parseInt(fileSizeOnly)
        }
        output = output / (1000 * 1000 * 1000)
        //console.log('final size of ', dirToCheck, output)
        return output;
        /*
    ...
    sh.fs.isDirBiggerThanXGBs = function isDirBiggerThanXGBs(dir, gb, fxCallback) {
        var size = sh.fs.getSizeOfDir3(dir)
        var sizeO = size
        //console.log('isDirBiggerThanXGBs', 'size', size, sizeO)
    ...
    sh.fs.addExt = sh.fs.addFileExt = function addFileExt(file, ext) {
        var filename = file;
        if (ext.startsWith('.')) {
            ext = ext.slice(1)
        }
    ...
    sh.fs.moveTo = function moveTo(file, dir, ext) {
        var leaf = sh.fs.leaf(file)
        var fileNew = sh.fs.join(dir, leaf)
        fileNew = sh.fs.addExt(fileNew, ext)
        return fileNew
    }
}
;
...
function defineRemote() {
    sh.get = function getUrl(url, fxDone, qs) {
        if (url.startsWith(':')) {
...
sh.sortByAlphaNumeric = function sortByAlphaNumeric(listItems, prop) {
    var collator = new Intl.Collator(undefined, {numeric: true, sensitivity: 'base'});
    //var myArray = ['1_Document', '11_Document', '2_Document'];
    listItems.sort(collator.compare);
...
sh.sortDict = function sortOnKeys(dict) {
...
function defineFSFunctions() {
    ...
    sh.fs.filenameAddToSubDir = function filenameAddToSubDir(file, subDirName) {
        var filename = sh.getFileName(file);
       // var filename2 = sh.getFileNameOnly(file);
       // var ext = sh.getFileExt(file);
        var path = sh.getPath(file);
        var dir = path+ '/' + subDirName + '/'

        sh.fs.mkdirp(dir)

        var line = dir + filename;
        return line;

    }


    sh.fs.filenameChangeExt = function filenameChangeExt(file, nextExt) {
        var filename = sh.getFileName(file);
        var filename2 = sh.getFileNameOnly(file);
        var ext = sh.getFileExt(file);
        if ( nextExt.startsWith('.')==false) {
            nextExt = '.'+nextExt;
        }
        var path = sh.getPath(file);

        var line = filename2 + nextExt;
        return line;

    }

    sh.fs.isFile = function isFile(file) {
        var fs = require("fs")
    ...
    sh.fs.changeFilename = function changeFile(file, extra, doFullCheck) {
        var fs = require("fs")
        doFullCheck = sh.dv(doFullCheck, true)
        if (doFullCheck) {
            if (sh.fs.isFile(file) == false) {
                sh.throw(file, 'is not a file')
                return true
            }
        }
        var filename = sh.getFileName(file);
    ...
    sh.fs.makePathToFile = function makePathToFile(file) {
        sh.fs.mkdirp(file, true)
    }
...
sh.fs.trash = sh.fs.getTrashDir = function getTrashDir(addToPath) {
...
sh.getContentAfter = function getContentAfter(url, findStr) {
...
sh.getContentBefore = function getContentBefore(url, findStr) {
    if (url.includes(findStr)) {
...
sh.str.before = sh.getContentBefore;
sh.str.after = sh.getContentAfter;

sh.str.cap = function cap(str) {

    //var arr = str.split(' ')
    var words = sh.str.splitStringIntoWords(str)
    var fixedWords = [];
    sh.each(words, function caplitali(k,word) {
        word = sh.capitalize(word)
...
sh.str.between = sh.getContentBetween;
...
function defineERrorHndlers() {

    sh.storeErrorsInLogFile = function storeErro(file) {
        file = sh.fs.leaf(file)
        var fs = require('fs');
        var util = require('util');
        var dirX = sh.fs.trash() + '/storeLogOutputs/'
        var fileErrors = dirX + '/' + file + '.error.log'
        sh.mkdirp(dirX)
        var log_file = fs.createWriteStream(fileErrors, {flags: 'w'});
        var log_stdout = process.stderr;
        console.log('will store error file in', fileErrors)

        var oldError = console.error;

        console.error = function error(d) { //
            var args = sh.args(arguments)
    ...
    sh.storeLogOutput = function storeErro(file) {
        var fs = require('fs');
        var util = require('util');
        var dirX = sh.fs.trash() + '/storeLogOutputs/'
        file = sh.fs.leaf(file)
        var fileErrors = dirX + file
        sh.mkdirp(dirX)
        var log_file = fs.createWriteStream(fileErrors, {flags: 'w'});
        // var log_stdout = process.stdout;


        console.log('will store file in', fileErrors)

        var oldError = console.error;
        var oldLog = console.log;
        console.error = function errorRedirct_output(d) { //
            var args = sh.args(arguments)
            var msg = args.join(' ')
            log_file.write('err>' + util.format(msg) + '\n');
            //  oldError.write(util.format(msg) + '\n');
            oldError.apply(console, args)
        };
...
sh.defaultValue2 = function addDefaultValueToObjIfNull(objInput, prop, ifNullUse) {
    if (objInput == null) {
        return;
    }
    if (objInput[prop] == null) {
        objInput[prop] = ifNullUse
    }
    return objInput;
}


sh.dv2 = sh.defaultValue2;
...
function arrayCollectProp(items, prop) {
    ...
    if (items.length == null) {
        var itemsOrig = items;
        var items = []
        sh.each(itemsOrig, function ok(k, v) {
            items.push(v)
        })
...
function filterPropsTo(from, filterPropList___) {
    if (sh.isArray(filterPropList___) == false) {
        var args = sh.args(arguments)
        filterPropList___ = args.slice(1)
    }
    var filterPropList = filterPropList___

    sh.each(from, function onFilterProps(prop, val) {
        var val = from[prop];
        if (filterPropList.includes(prop) == false) {
            from[prop] = null;
            delete   from[prop];
        }
    })
...
sh.filterPropsTo = filterPropsTo;
...
function defineEachMethods() {
    ...
    each.pushUnique = function pushUnique(items, add, addIn) {
    ...
    each.merge = function merge(from, to) {
        sh.each(from, function mergeInto(k, v) {
            to[k] = v;
        })
    }

    each.find = function find(items, lookFor, prop, throwI) {
        var found = false;
        sh.each(items, function printItem(k, v) {
            if (prop) {
                v = v[prop]
            ...
            if (v == lookFor) {
                if (throwI) {
                    sh.throw('found it in', lookFor, v)
                }
                found = true
                return false;
            }
    ...
    each.find2 = function find2(items, lookFor, prop, throwI) {
        var found = false;
        sh.each(items, function printItem(k, item) {
            var v = item;
            if (prop) {
                v = v[prop]
            }
            if (v.toLowerCase().includes(lookFor.toLowerCase())) {
                if (throwI) {
                    sh.throw('found it in', lookFor, v)
                }
                found = true
    ...
    each.number = function number(items, prop) {
        prop = sh.dv(prop, 'index')
        sh.each(items, function printItem(k, v) {
            v[prop] = k + 1
        })
    }
    ...
    each.keepIf = each.filterList = function filterList(array, field, query, asVal) {
        if (query == null)
            return array;
    ...
    each.filterIf = function filterIf(array, field, query, asVal) {
        if (query == null) {
            return array;
        }
    ...
    each.removeIfStr = each.filterIfStr = function filterIfStr(array, str) {
    ...
    each.copyArrayItemsToItemProp = function
        makeObjectFromArray_CopyItemsToPropertiesOnNewObject_ReturnsArray(items, prop, fxProc) {
        //prop = sh.dv(prop, 'id');
        var outputList = [];
        sh.each(items, function addToDict(i, obj) {
            ...
            if (sh.isArray(prop)) {
                var props = prop;
                sh.each(props, function copyEachProp(k, prop) {
                    newItem[prop] = obj[prop];
                })
            } else {
                newItem[prop] = obj
            }
    ...
    each.copyTo = function copyTo(copySrc, copyDest) {
        sh.each(copySrc, function addToDict(k, v) {
            copyDest[k] = v
            //outputList.push(newItem)
        });
    }

    each.getKeys = function getKeys(copySrc) {
        var keys = [];
        sh.each(copySrc, function addToDict(k, v) {
            keys.push(k)
            //outputList.push(newItem)
...
//AKA Lines helper
    ...
    function defineFx2() {
        each.propSearch = function onDoPropSearch(list, prop, searchFor, fxOk) {
            sh.throwIfNull(fxOk, 'val...')
            sh.each(list, function searchForx(k, item) {
                var val = item[prop];
                val = sh.dv(val, '')
                // console.log('val', 'what is nfl', val, prop)
                if (val.includes(searchFor)) {
                    fxOk(item)
                }
            })
        }

        each.removeField = function removeFields(items, name) {
            var props = sh.args(arguments).slice(1)
            sh.each(items, function removeField(k, item) {
                sh.each(props, function removeProp(k2, prop) {
                    item[prop] = null
                    delete item[prop]
        ...
        each.index = function index(items) {
            sh.each(items, function removeField(k, item) {
                item.index = k;
            })
        }
        ...
        each.removeDupes = function removeDupes(items, prop) {
            var dictFlagged = sh.dict();
            sh.each(items, function removeField(k, item) {
                var uniqID = item[prop];
                if (dictFlagged.includes(uniqID)) {
                    return;
                }
                dictFlagged.add(uniqID, item)
            })

            var output = dictFlagged.flatten();
            return output;
        }
...
function getTimeStamp(timeOverride, dateOnly) {
    var str = "";
    ...
    str = (currentTime.getMonth() + 1) + '-' + currentTime.getDate() + '-' + currentTime.getFullYear()
    if (dateOnly != true) {
        var timeStr = hours + "_" + minutes + "_" + seconds// + " ";
        str += '_' + timeStr;
    }
...
function timeElapsedMS(lastDate) {
    var currentTime = new Date()
    var time = currentTime.getTime() - lastDate.getTime()
    return time;
}
...
function wrapInHTMLTag(str, tag, styleObj) {
    let style = '';
    if ( styleObj ) {
        if ( sh.isString(styleObj)) {
            style = styleObj;
        }
        else {
            style = ''
            sh.each(styleObj, function styleO(k,v) {
                style += k + ': ' + v  +'; '
            })
        }
...
function EasyTimer() {
    ...
    this.ms = function ms() {
        var msElapsed = sh.timeElapsedMS(this.startTime)
        return msElapsed;
    }
    ...
    this.getTime = function getTime(msg) {
        this.duration = sh.timeElapsed(this.startTime).toFixed(2)
        msg = sh.dv(msg, '')
        console.log(msg, this.secs());

    }

    this.time = function howMuchTimeHasPaste(msg) {
        msg = sh.dv(msg, '')
        console.log(msg, this.secs());
    }
...
sh.ifDoesNotHaveProp = function ifDoesNotHaveProp(obj, prop, msg) {
    var val = obj[prop]
    if (val === undefined || val == null) {
        if (msg) {
            console.log('ifDoesNotHaveProp', msg, sh.qq(prop), val)
        }
        return true
    }
...
sh.toPer2 = sh.toPercent2 = function toPercent2(a, b, extened) {
    if (a.length) a = a.length;
    if (b.length) b = b.length;

    var str = [sh.toPercent(a / b), sh.paren([a, 'of', b,].join(' '))].join(' ')
    return str;
}

sh.toNum = function toNum(number) {
    if (number == null) {
    }
...
sh.inc = function incrementIfValid(sum, add, warning, add2) {
    if (add && sh.isNumber(add)) {
        sum += add
    } else {
        if (warning) {
            console.log('warrning adding', warning, add, add2)
        }
    }
    return sum;
}
...
sh.toCol = function toCOlumns(obj, display) {
    display = sh.dv(display, false)
    return sh.printCol(obj, !display)
}
sh.toCSV = function toCSV(obj, display) {
    var json2csv = require('json2csv');
    var columns = json2csv({data: obj})
...
sh.run2 = function runSyncWinAsBatFile(cmd, opts) {
...
sh.runFailable = function runFailable(cmd, opts) {
    try {
        sh.run(cmd, opts, true)
    } catch (e) {
        console.error(e)
    }
...
sh.runAsync = function runCommandAsync(cmd, fxDone, opts) {
    ...
    function fxDoneRedirect(error, x, output) {
        if (error != null) {
            console.error('error sh.runAsync')
    ...
    ipAdd.stdout.on('data', function (data) {
        sh.cid(opts.fxEcho, data)
...
sh.async = function async(items, fxAction, fxDone, concurrency) {
    ...
    async.forEachLimit(items, concurrency, function processEachTest(item, fxInteration) {
        ...
        itemIndex++
        fxAction(item, fxInteration, itemIndex);
...
sh.time.pTime = sh.time.hrTime = function hrTime(a, b, ms) {
    if (b == null) {
        b = new Date();
    }
    if (a == null) {
    ...
    var diff = b.getTime() - a.getTime()
    diff = sh.time.secs(a, b, ms)
    if (diff > 60) {
        var diffMins = diff / 60;
        diff = diffMins.toFixed(1);
        diff += 'm'
        if (diffMins > 60) {
...
sh.time.formatSecs = function formatSecs(secs) {
    // diff = sh.time.secs(a,b,ms)
    if (sh.isNumber(secs) == false) {
        return 'bad input  ' + secs
    }
    if (secs > 60) {
        var diffMins = secs / 60;
        secs = diffMins.toFixed(1);
        secs += 'm'
        if (diffMins > 60) {
            var diffHours = diffMins / 60;
            secs = diffHours.toFixed(1);
            secs += 'h'
        }
    } else {
        secs = secs.toFixed(0);
...
function testTimeFinctiosn() {
    var d = new Date();
    var l = sh.time.pTime(d)
    console.log(l)
    d.setTime(d.getTime() - 120 * 1000)
    var l = sh.time.pTime(d)
    console.log(l)
    d.setTime(d.getTime() - 1 * sh.time.hours)
    var l = sh.time.pTime(d)
    console.log(l)
    sh.x()
}
...
sh.moreThanZero = function numMoreThanZero(o) {
    if (o == 0) {
        return false
    }
    if (sh.isNumber(o)) {
...
sh.stack = function showStack(msg) {
    console.error('showStac', msg, new Error().stack)
}
...
function defineMiddleware() {
    ...
    sh.uploadFileHelper = function uploadFileHelper(cfg) {
        if (cfg.uploadPath.startsWith('/') == false) {
            sh.throw('bad path', path, 'need a / in front like /uploadFileLarge')
        }
        sh.throwIfNull(cfg.dirStore, 'where to store this?')
        cfg.dirStore += '/'
        sh.fs.mkdirp(cfg.dirStore)
        var multer = require('multer');
        var storage = multer.diskStorage({
            destination: function (req, file, cb) {
                cb(null, cfg.dirStore)			// DESTINATION FILES
            },
        ...
        cfg.app.post(cfg.uploadPath, upload.single('file'), function onUploadFinished(req, res) {

            console.log('file upload', req.file.filename)

            var filename = req.file.filename;
            var filePath = req.file.destination + req.file.filename
            sh.cid(cfg.fxDone, filePath, filename, req)
...
sh.catchErrors = function catchErrors(fxTest) {
    process.on('uncaughtException', function (err) {
        console.error('Caught exception: ' + err);
        console.error(err.stack)

        if (fxTest) {
            var result = sh.cid(fxTest, err)
            if (result != true) {
                throw err
            }
        }
...
function defineTreeMethods() {
    sh.tree = {}
    sh.tree.all = function visitAll(nodes, fx, childProp) {
        (function visit(obj) {
            //obj.mark = true;
            for (var key in obj) {
    ...
    sh.tree.all = function visitAll(nodes, fx, childProp) {
        childProp = sh.dv(childProp, 'children')
        function visit(obj) {
            //obj.mark = true;
            if ( sh.isArray(obj) == false ) {
            ...
            sh.each(children, function touchNode(k,child) {
                fx(child)
                visit(child)
            })
            /*for (var key in obj) {
    ...
    sh.tree.flat = function flattendTree(nodes) {
        var flattenedNodes = []
        sh.tree.all(nodes, function addNode(node) {
...
helper.deos = function deos(path) {
    if (path.includes('Dropbox')) {
        if (sh.isMac()) {

            console.log(__dirname, path)
            var postPath = sh.str.after(path, 'Dropbox')
            var postPre = sh.str.before(__dirname, 'Dropbox')
            path = postPre + 'Dropbox' + postPath
            console.log(path)
            // asdf.g
        }
    }
...
helper.removeWhitespace = function removeWhitespace(str) {
    str = str.replace(/\s/g, '')
    return str;
}

helper.removeNumbers = function removeNumbers(str) {
    str = str.replace(/[0-9]/g, '');
    return str;
}
helper.alpha = helper.removeNonAlpha = function removeNonAlpha(str) {
    str = str.replace(/[^0-9a-z]/gi, '')
    return str;
}
helper.alphaOnly   = function removeNonAlpha(str) {
...
helper.lLog = lLog;
helper.sLog = sLog;
helper.proc = function proc() {
    if (helper.procSilent == true) {
        return;
    }
    sh.sLog(arguments);
};
...
helper.fs.cp = copyFile;
helper.fs.cp2 = copyFile2;
...
helper.isAlphaNumeric = helper.isAlpha = helper.isChar = function isLetter(str) {
...
helper.later = function later(fx, time) {
    time = sh.dv(time, 10)
    setTimeout(fx, time)
}
...
helper.merge = helper.mergeObjects
...
helper.timeElapsedMS = timeElapsedMS;
helper.timestamp = helper.getTimeStamp = getTimeStamp
helper.getDatestamp = helper.getDate = function getDate() {
    return helper.timestamp(null, true)
}
