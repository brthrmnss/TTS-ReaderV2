...
rUtils.expandRectangle = function expandRectangle(div, divFrom) {
    ...
    var xDiff = (fromDim.maxX + 10 - 10) - toDim.maxX
    if (xDiff > 0) {
        div.width(div.width() + xDiff)
    }
    var yDiff = fromDim.maxY - toDim.maxY
    if (yDiff > 0) {
        div.height(div.height() + yDiff)
    }
...
rUtils.expandDimensions = function expandDimensions(box, percentage) {
...
//1.2
    ...
    pH.sortSpans = function sortSpans(_spans) {
        ...
        sSH.detectColumns_PACMode = function detectColumns_PACMode(___spans_dC) {
            ...
            colHelper.grabSpans = function grabSpans(col) {
                h.currentCol
                h.allSpans

            }
            colHelper.addCol = function addCol() {
            ...
            function ColHelper() {
                ...
                p.grabSpansForCol = function grabSpansForCol() {
                    if (self.data.currentCol) {
                        ...
                        colHelper.expandY(rect, origBox.maxX - 2, origBox.maxY, averageFontSize,
                            maxMisses, true)
            ...
            function proveScrollBarNeededForElementsFromPoint() {

                var x = 380
                var y = 190
                x = 38
                y = 110
                var col = rUtils.drawRectangle(x, y, 5, 5, 'purple');
                var realX = x + xyPage.left;
                var realY = y + xyPage.top;
                var col = rUtils.drawRectangle({
                    x: realX, y: realY, //',  //' - glOff.y,
                    root: true,
                    w: 15, h: 15, bg: 'green'
                });

                var col = rUtils.drawRectangle(38, 110, 10, 10, 'yellow');

                var off = window.pH.data.pageDim;
                console.error('where is point', xyPage, '[', window.pH.data.pageDim)
                off.left += x
                off.top += y
                console.error('where is point', off)

                /*realX += 2;
                 realY += 2;*/


                var testMark = document.elementsFromPoint(realX, realY)
                console.error('1111', testMark, realX, realY)

                var testMark = document.elementsFromPoint(realX, realY + glOff.y)
                console.error('2222', testMark, realX, realY - glOff.y)

                var testMark2 = document.elementsFromPoint(x, y)
                console.error('3333', testMark2, x, y)
                return;
            ...
            //proveScrollBarNeededForElementsFromPoint()
            //return;
            ...
            for (var x = rip.settings.margin.minX; x < pH.pageWidth; x = x + jumpX) {
                ...
                for (var y = rip.settings.margin.minY; y < pH.pageHeight; y = y + jumpY) {
                    //console.log('y', y)
                    ...
                    if (rUtils.inRange(x, y, rip.settings.margin)) {
                        ...
                        if (origFont.length > 0) {
                            ...
                            if (colHelper.data.cols.length > 2) {
                                debugger;
                            }

                            var col = rUtils.drawRectangle(x, y, 5, 5, 'green');
                            colHelper.grabSpansForCol()
                            colHelper.addCol(col)
                            rUtils.expandRectangle(col, origFont[0]);
                            cols.push(col)

                            var colDim = rUtils.getSpanDimensions(col);
                            console.debug('size of col', col, colDim)
                            ...
                            for (var x2 = x + jumpX2; x2 < pH.pageWidth; x2 = x2 + jumpX2) {
                                ...
                                var files = document.elementsFromPoint(realX2 + glOff.x, realY2 + glOff.y)
                                var origFont2 = $(files).filter('[origfont]')
                                if (origFont2.length > 0) {
                                    var div = rUtils.drawRectangle(x2, y, 5, 5, 'yellow');
                                    rUtils.expandRectangle(col, origFont2[0]);
                            ...
                            colHelper.expandY = function expandY(divCol, x, y, yMove, maxMissesAllowed, dbgAll) {
                                var missedCount = 0;
                                var totalMissedCount = 0;
                                for (var y2 = y; y2 < pH.pageHeight; y2 = y2 + yMove) {
                                    var realX2 = x + xyPage.left;
                                    var realY2 = y2 + xyPage.top;
                                    var elementsUnderPt = document.elementsFromPoint(realX2 + glOff.x, realY2 + glOff.y)
                                    var origFont2 = $(elementsUnderPt).filter('[origfont]')
                                    if (origFont2.length > 0) {
                                        //asdf.g
                                        var div = rUtils.drawRectangle(x, y2, 5, 5, 'gray');
                                        rUtils.expandRectangle(divCol, origFont2[0]);
        ...
        if (rip.settings.tryToDetect3Columns) {
            if (rip.settings.tryToDetect3ColumnsExt) {
                _spans = sSH.detectColumns_PACMode(_spans);
