...
//		console.log('color here....', 'where are tranforms made?')
	...
	PDFPage.prototype = {
		...
		resize: function (scale) {
			...
			this.textLayer.bind("mouseup", function(){
				/**console.log("j'ai mouseup2 sur textLayer");
				$('#XLayer').find('span.highlight').each(function(){
					var $this=$(this);
					$this.replaceWith($this.textContent);
				});

				var selection = rangy.getSelection(this.textLayer);
...
//bookmark.before rendering
	...
	PDFViewerAPI.prototype = {
		...
		getPageContent: function () { // XWZ
			if(this.viewer.pdf === null) {
				return "";
			}
			console.log("pages: "+this.viewer.pages.length);
			console.log("currentPage: "+this.viewer.currentPage);
			
			//return this.viewer.pages[this.viewer.currentPage];
		...
		return {
			...
			controller: ['$scope', '$element', '$timeout', function ($scope, $element) {
				...
				$scope.onCurrentPageChanged = function (pageID) {
					...
					$timeout(function() {
						$scope.onProgress("pageChanged", 'changed', pageID ); //, total, message);
					})
				};
				...
				$element.bind("click", function (event) {
					/**console.log("je suis ds pdfJS directive");
			...
			link: function (scope, element, attrs) {
				...
				//scope.$watch("searchTerm", function (searchTerm) {
					if (searchTerm !== undefined && searchTerm !== null && searchTerm !== '') {
						...
						scope.viewer.search(searchTerm);
						console.log('je Suis dans searchTerm.$observe');
					} else {
		...
		return {
			...
			link: function(scope, element, attrs){
				element.bind('click', function(){
					console.log("je suis ds layers");
					
					var selection = rangy.getSelection(layer);
					console.log("selection : "+selection);
				})
			}
		}
