...
function ProcessPdfBook() {
    var p = ProcessPdfBook.prototype;
    p = this;
    var self = this;
    self.data = {};
...
//file = "G:/Dropbox/projects/delegation/Reader/TTS-Reader/uploads/extracted/testquotesepub/epub_text.html"
    ...
    p.defineUtils = function defineUtils() {
    ...
    p.defineRUtils = function defineRUtils() {
        ...
        rUtils.drawBlock = function drawBlock(size, name, size2) {
            ...
            // parent.append(div2)
            // margins.minX = width;
            window.pH.data.ui.append(div2)

            return div2
        }

        rUtils.expandRectangle = function expandRectangle(div, divFrom) {
            //var posSentenceDiv = pH.utils.getPos(divFrom)
            divFrom = $(divFrom);
            window.upgradeSpanTo(divFrom);
            var fromDim = divFrom.offset();
            //off
            fromDim.maxX = fromDim.left + divFrom.width();
            fromDim.maxY = fromDim.top + divFrom.height();

            div = $(div);
            window.upgradeSpanTo(div);
            var toDim = div.offset();
            //off
            toDim.maxX = toDim.left + div.width();
            toDim.maxY = toDim.top + div.height();

            //console.log(div[0], divFrom[0], toDim, fromDim  )

            var xDiff = fromDim.left - toDim.left
            if (xDiff < 0) {
                toDim.left = fromDim.left
                div.offset(toDim)
                //div.width(div.width() + xDiff)
                div.width(div.width() + (-1 * xDiff )) //have to expand to maek sure maxX does not chnage
            }
            var yDiff = fromDim.top - toDim.top
            if (yDiff < 0) {
                toDim.top = fromDim.top
                div.offset(toDim)
                div.height(div.height() + (-1 * yDiff ))
            }
            ...
            //console.log('sdf', toDim)

            var xMaxDiff = (fromDim.maxX + 10 - 10) - toDim.maxX
            if (xMaxDiff > 0) {
                div.width(div.width() + xMaxDiff)
            }
            var yMaxDiff = fromDim.maxY - toDim.maxY
            if (yMaxDiff > 0) {
                div.height(div.height() + yMaxDiff)
            }

        }

        rUtils.drawRectangle = function drawRectangle(cfg, y, w, h, bg) {
            /*if ( cfg.x == null) {
             console.warn ('issue with this')
             return ;
             } */
            if (cfg.x == null) {
                cfg = {x: cfg, y: y, w: w, h: h}
            ...
            var div2 = $('<div/>')
            // div2 = div.clone();
            cfg.backgroundColor = sh.dv(cfg.backgroundColor, bg)
            cfg.backgroundColor = sh.dv(cfg.bg, cfg.backgroundColor)
            cfg.backgroundColor = sh.dv(cfg.backgroundColor, 'red');

            div2.css('background-color', cfg.backgroundColor)

            div2.css('opacity', '0.5')
            //div2.text(name)
            //div2.css('left', size.x + 'px')
            cfg.width = sh.dv(cfg.width, cfg.w)
            div2.css('width', cfg.width + 'px')
            div2.css('top', cfg.y + 'px')

            if (cfg.x) {
                div2.css('left', cfg.x + 'px')
            }

            cfg.height = sh.dv(cfg.height, cfg.h)
            div2.css('height', cfg.height + 'px')
            if (cfg.y) {
                div2.css('top', cfg.y + 'px')
            }
            //console.log2('-------||', '---size', size)

            div2.css('position', 'absolute')
            if (cfg.addClass) {
                div2.addClass(cfg.addClass)
            } else {
                div2.addClass('annottatino_Columns')
            }
            //asdf.g
...
//        asdf.g.g
            ...
            // window.pH.data.ui.append('sdf')
            return div2
        }


        function defineRutils_BoxSearch() {
            rUtils.searchForSpansInArea2 = function findItemsInBox(spansToSearch, box) { //searchForSpansInArea
                var dictX = {}
                var dictY = {}
                var fxName = 'detectColumns.searchForSpansInArea2';
                if (window.dbgExpand)
                    console.log(fxName, 'find a box for', box, spansToSearch.length)
                ...
                $.each(spansToSearch, function filterSpansInMargin(k, span) {
                    ...
                    var searchStr = 'PREFACE'
                    searchStr = 'has alwags'
                    if (spanText.includes(searchStr)) {
                        if (window.dbgExpand)
                            console.log('\t', fxName, spanText, xy)
                    }
                    if (xy.x >= box.x) {
                        if (xy.x <= box.maxX) {
                            if (xy.y >= box.y) {
            ...
            rUtils.determineAverageFontSize = function determineAverageFontSize(spans) {
                var dbg = false;
                var tagName = 'determineAverageFontSize'
                var result = {};
                result.averageFontSize_PerSpan = 0;
                result.averageFontSize_PerChar = 0;
                var charLength = 0;
                $.each(spans, function onF(kk, vv) {
                    var ui = $(vv);
                    var fontSize = rUtils.getPx(ui.css('font-size'), true);
                    // fontSize = parseFloat(fontSize)
                    var text = ui.text();
                    ...
                    result.averageFontSize_PerSpan += fontSize
                    result.averageFontSize_PerChar += allFontSizePerChar
                })

                result.averageFontSize_PerSpan = result.averageFontSize_PerSpan / spans.length;
                result.averageFontSize_PerChar = result.averageFontSize_PerChar / charLength;
                result.averageFontSize = result.averageFontSize_PerChar
                console.log2(tagName, 'avgs', result.averageFontSize_PerSpan, result.averageFontSize_PerChar)
                //asdf.g
                if (result.averageFontSize == 0) {
                    asdf.g
                }
                return result;
        ...
        rUtils.getSpanDimensions = function getSpanDimensions(ui) {
            ...
            styles.minX = styles.left = styles.x = rUtils.clearPixels(styles.x)
            styles.y = ui.css('top')
            // console.log2('detectColumns', styles.y)
            styles.minY = styles.top = styles.y = rUtils.clearPixels(styles.y)
            /* tyles.x = ui.css('left')
             styles.y = ui.css('top')
        ...
        rUtils.getSpanDimensionsBoxCorners = function getSpanDimensionsBoxCorners(ui) {

            var cornerStyles = {}
            cornerStyles.tr = {x: ui.x, y: ui.y}
            cornerStyles.br = {x: ui.x, y: ui.y + ui.height}
            cornerStyles.tl = {x: ui.x + ui.width, y: ui.y}
            cornerStyles.bl = {x: ui.x + ui.width, y: ui.y + ui.height}

            return cornerStyles;
        }

        rUtils.expandDimensions = function expandDimensions(box, percentage, okIfNot) {
            if (box.maxX == null) {
                throw 'not a valid box'
            }
            if (window.dbgExpand)
                console.log('box1-expand pre', box)
            var moveBy = box.width * (percentage / 100)
            box.left = box.x = box.x - moveBy
            box.width = box.width + ( 2 * moveBy )
            box.maxX = box.right = box.right + moveBy

            var moveByY = box.height * (percentage / 100)
            box.height = box.height + ( 2 * moveByY )
            box.top = box.y = box.y - moveByY
            box.maxY = box.bottom = box.bottom + moveByY;

            if (window.dbgExpand)
                console.log('box-expand', box)
            return box;
        }
        rUtils.expandSpanDimensions = function expandSpanDimensions(span, percentage, okIfNot) {
            // console.log('box1', box)
            var box = rUtils.getSpanDimensions(span)
            var moveByX = box.width * (percentage / 100)
            box.left = box.x = box.x - moveByX
            box.width = box.width + ( 2 * moveByX )
            box.maxX = box.right = box.right + moveByX
            ...
            if (window.dbgExpand)
                console.log('expandSpanDimensions', box)

            span.width(box.width)
            span.height(box.height)
            span.top(box.top)
            span.left(box.left)
    ...
    p.defineUploadFxs = function defineUploadFxs() {
...
//debugger
        ...
        window.uploadHTMLForPage = function uploadHTMLForPage(_fxPageComplete, processPage2) {
            //console.clear();
...
//    data.htmlContents = window.pH.cSH.pageHTML;
    ...
    p.defineBookSettingOverrides = function defineBookSettingOverrides(rip) {
        ...
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 5, r: 8, b: 10};
        bookSettings.margin.b = 10
        bookSettings.startOnPage = null;
        rip.dictSettings['Life at the Top  New York’s Exceptional Apartment Buildings Kirk Henckels 312p_0865653402 (NXPowerLite Copy).pdf'] = bookSettings;
...
//bookSettings.tryToDetect3Columns = true;
        ...
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 3, t: 5, r: 5, b: 6};
        bookSettings.startOnPage = null;
        bookSettings.onlyExpectColumnWide = true
        bookSettings.tryToDetect3ColumnsExt = true;
        rip.dictSettings['BUILT ON WATER LISA BAKER 276P (NXPowerLite Copy).pdf'] = bookSettings;


        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 8, r: 9, b: 10};
        bookSettings.margin.b = 3
        bookSettings.startOnPage = null;
        rip.dictSettings['[Michael_Strangelove]_Post-TV_Piracy,_Cord-Cuttin(book4you.org).pdf'] = bookSettings;


        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 5, r: 9, b: 7};
        bookSettings.startOnPage = null;
        rip.dictSettings['[Wheeler_Winston_Dixon]_Streaming_Movies,_Media,_(book4you.org).pdf'] = bookSettings;


        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 14, r: 9, b: 14};
        bookSettings.startOnPage = null;
        rip.dictSettings['The Darknet- A Digital Copyright Revolution.pdf'] = bookSettings;

        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 10, r: 9, b: 10};
        bookSettings.margin.b = 7
        bookSettings.startOnPage = null;
        rip.dictSettings['[Jane_Hathaway]_El-Hajj_Beshir_Agha(book4you.org).pdf'] = bookSettings;

        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 7, r: 9, b: 10};
        bookSettings.margin.b = 10
        bookSettings.startOnPage = null;
        rip.dictSettings['[Anja_Schwarz,_Lars_Eckstein_(eds.)]_Postcolonial_(book4you.org).pdf'] = bookSettings;


        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 4, t: 5, r: 4, b: 10};
        bookSettings.margin.b = 6
        bookSettings.startOnPage = null;
        rip.dictSettings['[B.R._Burg]_Boys_at_Sea_Sodomy,_Indecency,_and_Co(book4you.org).pdf'] = bookSettings;


        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 9, r: 9, b: 10};
        bookSettings.margin.b = 9
        bookSettings.startOnPage = null;
        rip.dictSettings['[Jason_Farman]_The_Mobile_Story_Narrative_Practic(book4you.org).pdf'] = bookSettings;

        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = false;
        bookSettings.tryToDetect2Columns = false;
        bookSettings.margin = {l: 8, t: 8, r: 9, b: 10};
        bookSettings.margin.b = 7.5
        bookSettings.startOnPage = null;
        rip.dictSettings['[Ravindra_S._Goonetilleke]_The_Science_of_Footwear(book4you.org).pdf'] = bookSettings;

        var bookSettings = {};
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = false;
        bookSettings.margin = {l: 8, t: 5, r: 7, b: 10};
        bookSettings.margin.b = 10
        bookSettings.startOnPage = null;
        rip.dictSettings['coming_up_short_working_class_.pdf'] = bookSettings;
        ...
        //bookSettings = sh.clone(bookSettings)
        bookSettings.tryToDetect3Columns = true;
        bookSettings.margin = {l: 8, t: 5, r: 9, b: 8};
        //bookSettings.margin.b = 10
        bookSettings.startOnPage = null;
        rip.dictSettings['[Wendy_Hui_Kyong_Chun,_Thomas_W._Keenan]_New_Media(book4you.org).pdf'] = bookSettings;

    }

//bookSettings.margin = {l: 2, t: 4, r: 2, b: 3};


    p.convertFile = function convertFile(config) {

        var cheerio = require('cheerio');

        var sh = require('shelpers').shelpers;
        var shelpers = require('shelpers');
        //var nlp = require('./r3.js')
        var nlp = require('compromise')

        window = {}

        window.upgradeSpanTo = function upgradeSpanTo(span) {
            if (span.width == null) {
                span.width = function getWidth(newWidth) {
                    if (newWidth) {
                        span.css('width', newWidth + 'px');
                        return;
                    }
                ...
                span.height = function get_height(newHeight) {
                    if (newHeight) {
                        span.css('height', newHeight + 'px');
                        return;
                    }
                ...
                function getOrSetProp(val, prop) {
                    if (val) {
                        span.css(prop, val + 'px');
                        return;
                    }
                    var h = span.css(prop);
                    h = parseFloat(h.replace('px', ''))
                ...
                span.top = function getTop(val) {
                    return getOrSetProp(val, 'top')
                }

                span.left = function getLeft(val) {
                    return getOrSetProp(val, 'left')
                }


                span.offset = function offset(newOffset) {
                    if (newOffset) {
                        span.css('top', newOffset.top + 'px');
                        span.css('left', newOffset.left + 'px');
                        // h.left = parseFloat(span.css('left').replace('px', ''))
                        // h.top = parseFloat(span.css('top').replace('px', ''))
                        return;
                    }
                    var h = {}
                    h.left = parseFloat(span.css('left').replace('px', ''))
        ...
        var contentsHTML = '<div id="txtWrapper">' + self.data.contents + '</div>'
        var $ = cheerio.load(contentsHTML);


        global.$ = $; //nodejs share var
...
// alert('whs')
        ...
        rip.dictSettings = {};
        self.defineBookSettingOverrides(rip);


//rip.settings.margin.t = 20

        var rUtils = self.defineRUtils();

        self.defineUploadFxs();
        ...
        function createTransportPanel() {
            ...
            window.uiUtils.br()
            window.uiUtils.addBtn({text: '0'},
                function onScanBookForREadableSentences(startReadingWhenProcesse) {
                    $scope.pdfViewerAPI.goToPage(3)
                    // $scope.pdfViewerAPI.goToPage(1)
                });
            window.uiUtils.addBtn({text: 'UploadAll'},
                function onScanBookForREadableSentences(startReadingWhenProcesse) {
                    window.uploadEntireBook()
                });
...
//window.showImages2 = true;
        window.fx2 = function fx2(play, playIndex) {
            ...
            pH.data.bookname = window.$scope.pdfURL.split('/').slice(-1)[0];
...
//1.2
            ...
            //window.overrideSet={};
            window.setMargins = function setMargins(l, t, r, b, col) {
                var y = {};
                y.l = l
                y.t = t;
                y.r = r;
            ...
            if (window.overrideSet) {
                $.each(window.overrideSet, function ok(k, v) {
                    rip.settings[k] = v;
                })
            }
            ...
            if (window.nodeJS !== true) {
                pH.totalPageHeight = $($(spans[0]).parent()).height();
            } else {
                pH.totalPageHeight = pH.pageHeight
            }
            ...
            pH.createPage = function createPage() {
                ...
                if (window.nodeJS !== true) {
                    console.log('made the page', pH.data.ui.offset())
                    window.pH.data.pageDim = pH.data.ui.offset(); //why: is this differnt values later on?
                } else {
                    pageCustom = page_
                    var div = uiUtils.tag('div')
                    div.attr('id', 'dfsdf')
                    $('.page').append(div)
                    window.pH.data.ui = div
                }
                console.log(' window.pH.data.pageDim ', window.pH.data.pageDim)
            ...
            function defineSortS() {
                ...
                utilsSort.putSpansIntoRows = function putSpansIntoRows(__Allspans) {
                    ...
                    $.each(/*_spans,*/ __Allspans, function arrangeSpansIntoRows(indexSpan, v) {
                        var span = $(v);
                ...
                utilsSort.flattendRows = function flattendRows(dictSpansByRow, clearAll) {
                    if (dictSpansByRow.spans) {
                        cfg = dictSpansByRow
                        dictSpansByRow = cfg.spans
                    } else {
                        cfg = {}
                    }
                    var spans = [];
            ...
            pH.sortSpans = function sortSpans(_spans) {
                ...
                var utilsSort = window.utilsSort;
                ;
                /*
                ...
                sSH.addMargins = function addMargins(_spans_, margins) {
                    ...
                    if (window.nodeJS !== true) {
                        ...
                        var parentWidth = window.pH.pageWidth;
                        var parentHeight = window.pH.pageHeight;
                    }
                    ...
                    if (margins.l) {
                        ...
                        div2.css('left', '0px')
                        var width = parentWidth * (margins.l / 100);
                    ...
                    if (margins.t) {
                        ...
                        var height = parentHeight * (margins.t / 100);
                    ...
                    if (margins.r) {
                        ...
                        var marginWidth = parentWidth * (margins.r / 100);
                        margins.maxX = parentWidth - width;
                        div2.css('width', marginWidth + 'px')
                        parent.append(div2)
                    }
                    ...
                    if (margins.b) {
                        ...
                        var marginHeight = parentHeight * (margins.b / 100);
                        div2.css('height', marginHeight + 'px');
                ...
                sSH.detectColumns = function detectColumns(___spans_dC) {
                    ...
                    if (rip.settings.onlyExpect1Column) {
                            ...
                            $.each(colSpans, function onRemoveIfBad(k, _span) {
                                ...
                                $.each(dictSpansAtRow_ByX, function onGoThrough(k, v) {
                                    ...
                                    if (spanPos2.left > xMin && spanPos2.left < xMax) {
                                        //debugger
                                        //console.debug('remove it', span.text(), 'from', v.text(), span[0], v[0])
                                        remove = true
                                        return false;
                                    }
                                })
                    ...
                    if (rip.settings.onlyExpectColumnWide) {
                        $.each(xVals, function onRemoveItems_FromCol_ThatDoNotIndicateColumn(xVal, colSpans) {
                            ...
                            $.each(colSpans, function onRemoveIfBad(k, _span) {
                                ...
                                $.each(dictSpansAtRow_ByX, function onGoThrough(k, v) {
                                    ...
                                    if (spanPos2.left > xMin && spanPos2.left < xMax) {
                                        //debugger
                                        console.log2('remove it', span.text(), 'from', v.text(), span[0], v[0])
                                        remove = true
                                        return false;
                                    }
                                })
                    ...
                    rUtils.defineSearchFx = function defineSearchFx(___spans_dC2) {
                        ...
                        rUtils.searchForSpansInArea = function findItemsInBox(spansToSearch, box) { //searchForSpansInArea
                            var dictX = {}
                            var dictY = {}
                            var fxName = 'detectColumns.searchForSpansInArea';
                            if (window.dbgExpand) {
                ...
                sSH.detectColumns_PACMode = function detectColumns_PACMode(___spans_dC) {
                    //if two rows appear ... then two rows
                    var fxName = 'detectColumns';

                    var dictSpansByRow = {}

                    var colPACModeSearch = ___spans_dC.concat()
                    window.colPACModeSearch = colPACModeSearch
                    var xVals = {};
                    ...
                    rUtils.inRange = function inRange(x, y, margins) {

                        var dbg = false;

                        if (margins.minX) {
                            if (x < margins.minX) {
                                if (dbg) console.log('x', x, '<', margins.minX)
                                return false;
                            }
                        }
                        ...
                        if (margins.minY) {
                            if (y < margins.minY) {
                                if (dbg) console.log('y', y, '<', margins.minY)
                                return false;
                            }
                        }
                        ...
                        if (margins.maxX) {
                            if (x > margins.maxX) {
                                if (dbg) console.log('x', x, '>', margins.maxX)
                                return false;
                            }
                        }
                        ...
                        if (margins.maxY) {
                            if (y > margins.maxY) {
                                if (dbg) console.log('y', y, '>', margins.maxY)
                                return false;
                            }
                        }
                        return true;
                    ...
                    rUtils.inBox = function inBox(x, y, margins) {
                        var dbg = false;
                        ...
                        var dbgPt = rUtils.drawRectangle(x, y, 10, 10, 'orange');
                        dbgPt.css('z-index', 1000)
                        dbgPt.addClass('inBoxDbg')

                        var fxName = 'iBbox'

                        if (dbg) {
                            console.log(fxName, x, y, '|', margins.x, margins.maxX, margins.y, margins.maxY)
                            console.log('\t', fxName, '|', margins.x, '-', x, '-', margins.maxX,
                                '|', margins.y, '-', y, '-', margins.maxY)

                        }

                        if (margins.x) {
                            if (x > margins.x) {
                                if (dbg) console.log(fxName, 'x', x, '<', margins.x)

                                if (margins.maxX) {
                                    //debugger
                                    if (x < margins.maxX) {
                                        if (dbg) console.log(fxName, 'x', x, '>', margins.maxX)
                                        if (margins.y) {
                                            if (y > margins.y) {
                                                // debugger
                                                if (dbg) console.log(fxName, 'y', y, '<', margins.y)
                                                if (margins.maxY) {
                                                    if (y < margins.maxY) {
                                                        if (dbg) console.log(fxName, 'y', y, '>', margins.maxY)
                                                        return true;
                                                    }
                                                }
                            ...
                            console.warn('bad input', margins)
                        }

                        return false;
                    }

                    console.log('ok', pH.pageHeight)

                    // var xyPage = JSON.parse(JSON.stringify(window.pH.data.pageDim));

                    if (window.nodeJS !== true) {
                        var xyPage = window.pH.data.ui.offset();
                    } else {
                        xyPage = {}
                        xyPage.top = 0
                        xyPage.left = 0
                        ...
                        var dictItems = []
                        $.each(colPACModeSearch, function findForX(k, v) {
                            var span = $(v)
                            rUtils.expandSpanDimensions(span, 5);
                            rUtils.getSpanDimensions(span);
                            var xy = rUtils.getSpanDimensions(span);
                            //sh.copyProps(xy, span) //, 'x', 'y', )
                            span.xy = xy;
                            dictItems.push(span)
                        })
                        window.getElementsFromPoint = function getElementsFromPoint(x, y) {
                            var matches = [];
                            $.each(dictItems, function findForX(k, span) {
                                //var xy = rUtils.getSpanDimensions(span);
                                //sh.copyProps(xy, span, 'x', 'y', )
                                if (rUtils.inRange(x, y, span.xy)) {
                                    matches.push(span)
                                }
                            })
                            return matches;
                    ...
                    if (window.nodeJS !== true) {
                        glOff.x = -$('body').scrollLeft()
                        glOff.y = -$('body').scrollTop()
                    } else {
                        glOff.x = 0
                        glOff.y = 0
                    }
                    ...
                    function ColHelper() {
                        ...
                        self.data.d = {};
                        self.data.cols = []; // = {};
                        p.addCol = function addCol(x, y, expandTo) {

                            var col = rUtils.drawRectangle(x, y, 5, 5, 'green');
                            col.css('font-size', '35px')

                            rUtils.expandRectangle(col, expandTo);

                            var cC = {};
                            cC.rect = col;
                            cC.box = rUtils.getSpanDimensions(col);
                            cC.boxCorners = rUtils.getSpanDimensionsBoxCorners(cC.box);
                            var found = false;

                            var pt = cC.boxCorners.tr
                            var tr = self.inACol(pt.x, pt.y)
                            var pt = cC.boxCorners.tl
                            var tl = self.inACol(pt.x, pt.y)
                            var pt = cC.boxCorners.br
                            var br = self.inACol(pt.x, pt.y)
                            var pt = cC.boxCorners.bl
                            var bl = self.inACol(pt.x, pt.y)

                            var pt = cC.boxCorners.tr
                            var tr = self.inACol(pt.x - 10, pt.y + 10)
                            var pt = cC.boxCorners.tl
                            var tl = self.inACol(pt.x + 10, pt.y + 10)
                            var pt = cC.boxCorners.br
                            var br = self.inACol(pt.x - 10, pt.y - 10)
                            var pt = cC.boxCorners.bl
                            var bl = self.inACol(pt.x + 10, pt.y - 10)

                            var skipAdd = null


                            function expandInto(tr) {
                                // console.log('tr', tr.box)
                                rUtils.expandRectangle(tr.rect, col);
                                tr.box = rUtils.getSpanDimensions(tr.rect);
                                tr.boxCorners = rUtils.getSpanDimensionsBoxCorners(tr.box);
                                //  console.log('tr', tr.box)
                                skipAdd = tr;
                                return tr;
                            }
                            ...
                            if (tr || tl || br || bl) {
                                // console.log('merged', tr)
                                if (tr) {
                                    console.log('merged tr', tr.name)
                                    skipAdd = expandInto(tr)
                                    // asdf.g
                                    // debugger
                                    // asdf.g
                                }
                                if (tl) {
                                    console.log('merged tl', tl.name)
                                    skipAdd = expandInto(tl)
                                    //yyy.box = rUtils.getSpanDimensions(yyy.rect);
                                    // debugger
                                    // asdf.g
                                }
                                if (br) {
                                    console.log('merged br', br.name)
                                    skipAdd = expandInto(br)
                                }
                                if (bl) {
                                    console.log('merged bl', bl.name)
                                    skipAdd = expandInto(bl)
                                }
                                //yyy.box = rUtils.getSpanDimensions(yyy.rect);
                                //debugger
                                // asdf.g
                            }
                            // sh.each(cC.boxCorners, function checkEachPoint(k,pt) {
                            /*var yyy = self.inACol(pt.x, pt.y)
                             if ( yyy ) {
                             asdf.g
                             rUtils.expandRectangle(yyy.rect, cC.rect);
                             yyy.box = rUtils.getSpanDimensions(yyy.rect);
                             yyy.boxCorners = rUtils.getSpanDimensionsBoxCorners(yyy.box);

                             //rUtils.joinRectangles(pt)
                             found = cC
                             debugger
                             }*/
                            //   })

                            /*sh.each(self.data.cols, function onK(k,searchCol) {
                             console.log('boo', v);
                             sh.each(cC.boxCorners, function checkEachPoint(k,pt) {
                             var yyy = self.inACol(pt.x, pt.y)
                             if ( yyy ) {
                            ...
                            /*sh.each(self.data.cols, function onK(k,searchCol) {
                             ...
                             // var boxCorners = rUtils.getSpanDimensions(v.rect)
                             //debugger
                             })*/
                            ...
                            self.data.cols.push(cC)
                            self.data.currentCol = cC
                            cC.name = 'c' + (self.data.cols.length + 0)

                            col.text('clm ' + (colHelper.data.cols.length + 0 ))
                            col.css('font-size', '50px')
                            col.css('font-weight', 'bold')
                            col.css('font-size', '10.11px')
                            col.css('font-size', '35px')
                            col.css('color', 'red')

                            return cC
                        }
                        p.inACol = function inACol(x, y) {
                            var insideCol = false;
                            var inside = false;
                            window.colLenGt2 = false;
                            if (self.data.cols.length >= 2) {
                                window.colLenGt2 = true
                            }
                            //  debugger
                            $.each(self.data.cols, function okk(k, col) {
                                col.rect._pStyles2 = rUtils.getSpanDimensions(col.rect)
                                var inIt = rUtils.inBox(x, y, col.rect._pStyles2);
                                if (inIt) {
                                    //debugger;
                                    inside = true;
                                    insideCol = col;
                        ...
                        p.expandY = function expandY(divCol, x, y, yMove, maxMissesAllowed, dbgAll) {
                            var missedCount = 1;
                            //asdf.g
                            sh.throwIf(yMove == 0, 'too low')
                            var totalMissedCount = 0;
                            for (var y2 = y; y2 < pH.pageHeight; y2 = y2 + yMove) {
                                var realX2 = x + xyPage.left;
                                var realY2 = y2 + xyPage.top;
                                var elementsUnderPt = window.getElementsFromPoint(realX2 + glOff.x, realY2 + glOff.y)
                                if (elementsUnderPt.length > 0) {
                                    //asdf.g
                                    var div = rUtils.drawRectangle(x, y2, 5, 5, 'gray');
                                    rUtils.expandRectangle(divCol, elementsUnderPt[0]);
                                    missedCount = 1; //reset miss count
                                    // console.log('inner matches', div[0],  origFont2[0])
                                } else {
                                    if (dbgAll) {
                                        var div = rUtils.drawRectangle(x, y2, 5, 5, 'red');
                                        div.text(['expandY', missedCount, '/', maxMissesAllowed].join(','))
                                    }
                                    if (maxMissesAllowed) {
                                        if (maxMissesAllowed <= missedCount) {
                                            break;
                                        } else {
                                            missedCount++
                                            totalMissedCount++
                        ...
                        p.showPositionsOfColumns = function showPositionsOfColumns(x, y) {
                            var insideCol = false;
                            var inside = false;
                            window.colLenGt2 = false;
                            if (self.data.cols.length >= 2) {
                            ...
                            $.each(self.data.cols, function okk(k, col) {
                                ...
                                var dim = col.rect._pStyles2;
                                //console.log(sh.t, k+1, dim.x,dim.y, dim.maxX, dim.maxY)
                                console.log(sh.t, k + 1, dim.x, sh.paren(x), dim.maxX,
                                    dim.y, sh.paren(y), dim.maxY)
                            })
                            return insideCol;
                        }


                        p.grabSpansForCol = function grabSpansForCol() {
                            if (self.data.currentCol) {
                                self.currentCol
                                var filterList = [];
                                var currentCount = self.data.colPACModeSearch.length;
                                var rect = self.data.currentCol.rect;
                                //  sh.each( self.data.colPACModeSearch, function filterS(k,v) {

                                var origBox = rUtils.getSpanDimensions(rect)
                                var box = rUtils.getSpanDimensions(rect)

                                box = rUtils.expandDimensions(box, 1.5)

                                var filterList = rUtils.searchForSpansInArea2(self.data.colPACModeSearch, box)

                                //filterList.push(v)

                                var averageFontSize3 = rUtils.determineAverageFontSize(filterList)

                                var averageFontSize = averageFontSize3.averageFontSize;
                                //averageFontSize = origBox.height * .02
                                // })

                                if (window.dbgExpand)
                                    console.log('filterList.grabSpansForCol', filterList.length,
                                        currentCount - filterList.length, averageFontSize)

                                //var rowInfo = self.rowInfo();

                                //debugger
                                var maxMisses = 1
                                //maxMisses = 3
                                colHelper.expandY(rect, origBox.x, origBox.maxY,
                                    averageFontSize, maxMisses, true)
                                colHelper.expandY(rect, origBox.maxX - 2, origBox.maxY,
                                    averageFontSize,
                                    maxMisses, true)

                                //  console.log('arr-filterList', filterList)
                                //asdf.g
                            }
                        }

                        p.exportCols = function exportCols() {
                            var boxes = [];
                            sh.each(self.data.cols, function grabCols(k, col) {
                                col.box = rUtils.getSpanDimensions(col.rect);
                                boxes.push(col.box)
                            })
                            var content = sh.toJSONString(boxes)
                            console.log(content)
                        }
                        p.importCols = function importCols(cols) {
                            sh.each(cols, function grabCols(k, box) {
                                var col = {};
                                col.box = box;
                                self.data.cols.push(col)
                            })
                        }
                        ...
                        p.redressColoumns = function redressColoumns() {
                            ...
                            sh.each(self.data.cols, function grabCols(k, col) {
                               // if (k == 0) return
                                ...
                                ;
                                //  $('.spanSortAnnotations').remove();
                                var yDict = utilsSort.putSpansIntoRows(filterList);
                                console.log('col number', filterList.length)
                                console.log(sh.t, col.text)
                    ...
                    var colHelper = new ColHelper();
                    colHelper.data.colPACModeSearch = colPACModeSearch;
                        ...
                        [
                            {
                                ...
                                "left": 29.492,
                                "minX": 29.492,
                                "y": 103.19800000000001,
                                "top": 103.19800000000001,
                                "minY": 103.19800000000001,
                                "fontSize": "35px",
                                "fontSizeAsNumber": 35,
                                "width": 147.4,
                                "right": 176.892,
                                "maxX": 176.892,
                                "height": 45.099999999999994,
                                "bottom": 148.298,
                                "maxY": 148.298,
                                "text": "clm 1"
                            },
                            {
                                ...
                                "fontSizeAsNumber": 35,
                                "width": 300.954,
                                "right": 331.73400000000004,
                                "maxX": 331.73400000000004,
                                "height": 604.672,
                                "bottom": 806.668,
                                "maxY": 806.668,
                                "text": "clm 2"
                            ...
                            {
                                "x": 345.04400000000004,
                                "left": 345.04400000000004,
                                "minX": 345.04400000000004,
                                "y": 200.436,
                                "top": 200.436,
                                "minY": 200.436,
                                "fontSize": "35px",
                                "fontSizeAsNumber": 35,
                                "width": 296.7759999999999,
                                "right": 641.8199999999999,
                                "maxX": 641.8199999999999,
                                "height": 600.4259999999999,
                                "bottom": 800.862,
                                "maxY": 800.862,
                                "text": "clm 3"
                            },
                            {
                                ...
                                "left": 654.668,
                                "minX": 654.668,
                                "y": 199.18800000000002,
                                "top": 199.18800000000002,
                                "minY": 199.18800000000002,
                                "fontSize": "35px",
                                "fontSizeAsNumber": 35,
                                "width": 303.592,
                                "right": 958.26,
                                "maxX": 958.26,
                                "height": 599.1519999999999,
                                "bottom": 798.3399999999999,
                                "maxY": 798.3399999999999,
                                "text": "clm 4"
                            }
                        ]
                    ...
                    var stopAtMaxY = 10000;
                    //stopAtMaxY = 300
                    stopAtMaxX = 80
                    stopAtMaxX = pH.pageWidth * 0.5
                    dbgAllPts = false
                    var dbgAllPointsViusally = false;
                    dbgAllPointsViusally = true
                    var dbgTestMarkerOffset = false;
                    dbgTestMarkerOffset = true
                    var maxCols = 5;
                    maxCols = 7;
                    stopAtMaxX = null;
                    console.log('glOff', glOff)
                    //return
                    var jumpX2 = 15
                    var jumpY2 = 15
                    for (var x = rip.settings.margin.minX; x < pH.pageWidth; x = x + jumpX) {

                        if (stopAtMaxX && x > stopAtMaxX) {
                            console.error('max x', x, stopAtMaxX, pH.pageWidth)
                            return;
                        }
                        for (var y2 = rip.settings.margin.minY; y2 < pH.pageHeight; y2 = y2 + jumpY) {
                            //console.log('y', y)

                            y = y2

                            if (rUtils.inRange(x, y, rip.settings.margin)) {

                                if (dbgAllPointsViusally) {
                                    var testMarker = rUtils.drawRectangle(x, y, 5, 5, 'brown');
                                    testMarker.css('z-index', 1000)
                                    testMarker.css('opacity', 1)
                                }
                                //var files = gs (x,y)

                                var realX = x + xyPage.left;
                                var realY = y + xyPage.top;

                                sh.throwIf(isNaN(realX), 'got a bad real x')
                                var inCol = colHelper.inACol(x, y)
                                if (inCol) {
                                    continue;
                                }
                                ...
                                if (dbgTestMarkerOffset) {
                                    var testMarker = rUtils.drawRectangle(x, y, 5, 5, 'brown');
                                    testMarker.css('z-index', 1000)
                                    testMarker.css('opacity', 1)

                                    var testMarker = rUtils.drawRectangle(realX, realY, 5, 5, 'yellow');
                                    testMarker.css('z-index', 1000)
                                    testMarker.css('opacity', 1)

                                    var testMarker = rUtils.drawRectangle(realX + glOff.x, realY + glOff.y, 5, 5, 'pink');
                                    testMarker.css('z-index', 1000)
                                    testMarker.css('opacity', 1)
                                }

                                if (window.nodeJS !== true) {
                                    var files = document.elementsFromPoint(realX + glOff.x, realY + glOff.y)
                                    var origFont = $(files).filter('[origfont]')
                                } else {
                                    files = window.getElementsFromPoint(realX + glOff.x, realY + glOff.y)
                                    origFont = files;
                                }

                                //  console.log('y', origFont)

                                if (files.length > 0) {
                                    //debugger
                                    if (origFont.length > 0) {
                                        // debugger
                                    }
                                }

                                if (origFont.length > 0) {
                                    //anot = rUtils.drawRectangle(x, y, 5, 5);

                                    if (colHelper.data.debugatColNumber) {
                                        if (colHelper.data.debugatColNumber > 2) {
                                            debugger;
                                        }
                                    }
                                    ...
                                    colHelper.showPositionsOfColumns(x, y);


                                    var cC = colHelper.addCol(x, y, origFont[0])
                                    var col = cC.rect;
                                    colHelper.grabSpansForCol() //call again later, when done expanding
                                    //cols.push(col)

                                    var colDim = rUtils.getSpanDimensions(col);
                                    if (window.dbgExpand) {
                                        console.debug('size of col', col.html(), colDim)
                                    }

                                    /*var inCol =  colHelper.inACol(realX, realY)
                                     if ( inCol ) {
                                     continue;
                                     }*/

                                    //debugger
                                    if (maxCols && colHelper.data.cols.length > maxCols) {
                                        console.error('max cols reached')
                                        return;
                                    }

                                    /*var col = rUtils.drawRectangle({
                                     x: realX + glOff.x, y: realY,
                                     root: true,
                                     w: 15, h: 15, bg: 'green'
                                     });*/

                                    /*
                                     console.log('x', x, 'y', y, '-', anot[0])
                                     console.log('x', realX,  'y', realY)
                                     console.log('x', realX + glOff.x,  'y', realY + glOff.y)
                                     console.log('inner matches', anot[0], origFont[0])
                                     */

                                    //keep waling right
                                    for (var x2 = x + jumpX2; x2 < pH.pageWidth; x2 = x2 + jumpX2) {
                                        var realX2 = x2 + xyPage.left;
                                        var realY2 = y + xyPage.top;

                                        var hitSpans = window.getElementsFromPoint(realX2 + glOff.x, realY2 + glOff.y)
                                        if (hitSpans.length > 0) {
                                            var div = rUtils.drawRectangle(x2, y, 5, 5, 'yellow');
                                            rUtils.expandRectangle(col, hitSpans[0]);
                                            // console.log('inner matches', div[0],  origFont2[0])
                                        } else {
                                            break;
                                        }
                                    ...
                                    var maxMisses = 4
                                    colHelper.expandY(col, x, y, -1 * jumpY2, maxMisses)
                                    colHelper.expandY(col, x, y, jumpY2, maxMisses)

                                } else {

                                    if (dbgAllPts) {
                                        anot = rUtils.drawRectangle(x, y, 5, 5, 'pink');

                                        console.log('x', realX, x, 'y', realY, y, '-', anot[0])
                                    }
                                }
                                ...
                                //console.log('---', files )
                                if (stopAtMaxY && y > stopAtMaxY) {
                                    console.error('stopAtMaxY x', stopAtMaxY)
                                    return;
                                }
                            }
                            //
                    ...
                    var cols = colHelper.data.cols
                    colHelper.redressColoumns();
                    colHelper.exportCols()
                    //debugger
                ...
                if (rip.settings.useOldDetection) {
            ...
            pH.createSentences = function createSentence(spans) {
                ...
                if (window.nodeJS !== true) {
                    ...
                    cSH.totalPageHeight = window.pH.pageHeight;
                }
                ...
                $.each(spans, function processSpan(k, v) { //find spans
                    ...
                    cSH.addAVGapElement = function addAVGapElement(why, span) {
                         ...
                         */
                        console.log2('bad size', cSH.uiElements.length)
                    ...
                    cSI.addSentenceToList = function addSentenceToList(why, addCurrentSpan) {
                        ...
                        var dbgAddingSentences = false;
                        var dbgWhyAdding = false;
                        dbgWhyAdding = dbgAddingSentences;

                        console.log2(fxName3, 'breaking a line???', why, cSH.currentSentence, spanText)
                        ...
                        if (!cSH.sentenceNotEmpty() && isOnSameLine == false) {
                            ...
                            if (dbgAddingSentences) {
                                console.debug(fxName3, 'breaking a line???', why, spanText)
                                console.log2(fxName3, '.....', 'isonsame', isOnSameLine, why)
                            }
                        ...
                        var dbgStylesDiff = false;
                        var isDifferent = uiUtils.utils.stylesDifferent(uiEl, span, dbgStylesDiff)
                        if (dbgWhyAdding) {
                            asdf.g
                    ...
                    cSI.skipDivs = function skipDivs(_span) {
                        ...
                        if (styles.top / cSH.totalPageHeight < sDH.headerRatio) {
                            console.log2('skip title ledger ', cSI.currentText)
                            // addSentenceToList(); //end of line
                            return true
                        }
                        if (styles.top / cSH.totalPageHeight > sDH.footerRatio) {
...
exports.ProcessPdfBook = ProcessPdfBook;
